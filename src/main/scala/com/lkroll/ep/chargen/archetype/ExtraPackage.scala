...
class ExtraPackageTable(
  ...
  private def ensureArchetypeSkills(rand: Random): Option[Result] = {
    ...
    archetype match {
      ...
      case Archetype.Fighter => {
        ...
        } else if (!hasAnySkillAbove(30, blades, beamWeapons, kineticWeapons, seekerWeapons, sprayWeapons, throwingWeapons, unarmedCombat)) {
      ...
      case Archetype.Hacker => {
        if (!hasAnySkillAbove(30, infosec)) {
          ...
          val table: RollTableLike[CustomizationPackage] = Array(
            CustomizationPackages.computerTraining,
            CustomizationPackages.techTraining);
          val res = table.randomElement(rand).get;
          Some(res)
        } else {
          None
  ...
  lazy val customizationData: RollTable[CustomizationPackage] = {
    ...
    archetype match {
      case Archetype.Butterfly if isAsync => RollTable(
        (1 to 6) -> artist,
      ...
      case Archetype.Butterfly => RollTable(
        ...
        (71 to 85) -> connected,
        (86 to 100) -> socialButterfly)
      case Archetype.Fighter if isAsync => RollTable(
      ...
      case Archetype.Fighter => RollTable(
        ...
        (71 to 80) -> weaponsTraining,
        (81 to 100) -> martialArtsTraining)
      case Archetype.Hacker if isAsync => RollTable(
        (1 to 10) -> essentialSkills,
        (11 to 11) -> mentalist,
        (12 to 19) -> spacer,
        (20 to 30) -> gearhead,
      ...
      case Archetype.Hacker => RollTable(
        ...
        (81 to 90) -> lucky,
        (91 to 100) -> sneaker)
      case Archetype.Scientist if isAsync => RollTable(
        (1 to 5) -> artist,
