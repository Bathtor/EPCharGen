...
class AdultPathTable(val nextPath: AdultPathIndex, val allowAsync: Boolean, val isAsync: Boolean) extends Table {
  ...
  override def roll(rand: Random): Result = {
    ...
    val nextIndex = pathChoice match {
      case PathChoice.MostRecentBackground =>
        nextPath.i match {
  ...
  private[lifepath] def packageFromIndex(rand: Random, nextIndex: Int, pckind: String): Result = {
    nextIndex match {
      ...
      case 12 => {
        ...
        AdultPath(kind = pckind, path = PathGroup(12, "Customization", FactionPathIndex(-1)), pkg = Right(pkg.get))
      }
      case _ => ???
    }
  }
...
object AdultPathTable {
  ...
  def postFall(nextPath: AdultPathIndex, isAsync: Boolean): AdultPathTable =
    new AdultPathTable(nextPath, true, isAsync);
  ...
  private val data0: RollTable[PathChoice] = RollTable((1 to 3) -> PathChoice.MostRecentBackground,
  ...
  private val data2: RandomArray[ThreePackageGroup[FocusPackage]] =
    Array(academic, activist, botJammer, covertOps, explorer, genehacker, hacker, medic, scientist, techie);
  private val data3: RandomArray[ThreePackageGroup[FocusPackage]] =
    Array(activist, conArtist, dealer, face, investigator, journo, smartAnimalHandler, soldier, techie, thief);
  private val data4: RandomArray[ThreePackageGroup[FocusPackage]] =
    Array(assassin, conArtist, covertOps, dealer, egoHunter, enforcer, hacker, pirate, smuggler, thief);
  private val data5: RollTable[ThreePackageGroup[FocusPackage]] = RollTable((1 to 1) -> academic,
                                                                            (2 to 2) -> dealer,
                                                                            (3 to 4) -> face,
                                                                            (5 to 6) -> icon,
                                                                            (7 to 7) -> journo,
                                                                            ...
                                                                            (10 to 10) -> scientist);
  private val data6: RollTable[ThreePackageGroup[FocusPackage]] = RollTable((1 to 1) -> academic,
                                                                            (2 to 2) -> conArtist,
                                                                            (3 to 4) -> dealer,
                                                                            (5 to 5) -> face,
                                                                            (6 to 6) -> icon,
  ...
  private val data7: RandomArray[ThreePackageGroup[FocusPackage]] =
    Array(activist, bodyguard, botJammer, conArtist, enforcer, pirate, scavenger, smartAnimalHandler, smuggler, thief);
  private val data8: RollTable[ThreePackageGroup[FocusPackage]] = RollTable((1 to 1) -> assassin,
                                                                            (2 to 2) -> bodyguard,
                                                                            (3 to 3) -> covertOps,
                                                                            (4 to 4) -> egoHunter,
                                                                            (5 to 5) -> enforcer,
                                                                            ...
                                                                            (7 to 9) -> soldier,
                                                                            (10 to 10) -> spy);
  private val data9: RandomArray[ThreePackageGroup[FocusPackage]] = Array(academic,
                                                                          explorer,
                                                                          genehacker,
                                                                          investigator,
                                                                          medic,
                                                                          ...
                                                                          smartAnimalHandler,
                                                                          techie);
  private val data10: RollTable[ThreePackageGroup[FocusPackage]] = RollTable((1 to 1) -> botJammer,
                                                                             (2 to 2) -> egoHunter,
                                                                             (3 to 4) -> explorer,
                                                                             (5 to 5) -> pirate,
                                                                             (6 to 6) -> scavenger,
                                                                             ...
                                                                             (8 to 9) -> smuggler,
                                                                             (10 to 10) -> spy);
  private val data11: RollTable[ThreePackageGroup[FocusPackage]] = RollTable((1 to 1) -> botJammer,
                                                                             (2 to 2) -> explorer,
                                                                             (3 to 3) -> genehacker,
                                                                             (4 to 5) -> hacker,
                                                                             (6 to 6) -> scavenger,
